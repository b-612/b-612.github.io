...
(function () {
  ...
  var makeDescription = function makeDescription(descriptionData, element) {
    var description = [];
  ...
  var makeAvatar = function makeAvatar(imageElement, imageData) {
    imageElement.attr('src', "img/".concat(imageData, "-mob@1x.jpg"));
    imageElement.attr('srcset', "img/".concat(imageData, "-mob@2x.jpg 2x"));
    $(imageElement.parent()).find('source[media="(min-width: 768px)"][type != "image/webp"]').attr('srcset', "img/".concat(imageData, "-tablet@1x.jpg 1x, img/").concat(imageData, "-tablet@2x.jpg 2x"));
    $(imageElement.parent()).find('source[media="(min-width: 1300px)"][type != "image/webp"]').attr('srcset', "img/".concat(imageData, "-desktop@1x.jpg 1x, img/").concat(imageData, "-desktop@2x.jpg 2x"));
    $(imageElement.parent()).find('source[type="image/webp"][media != "(min-width: 768px)"][media != "(min-width: 1300px)"]').attr('srcset', "img/".concat(imageData, "-mob@1x.webp 1x, img/").concat(imageData, "-mob@2x.webp 2x"));
    $(imageElement.parent()).find('source[media="(min-width: 768px)"][type = "image/webp"]').attr('srcset', "img/".concat(imageData, "-tablet@1x.webp 1x, img/").concat(imageData, "-tablet@2x.webp 2x"));
    $(imageElement.parent()).find('source[media="(min-width: 1300px)"][type = "image/webp"]').attr('srcset', "img/".concat(imageData, "-desktop@1x.webp 1x, img/").concat(imageData, "-desktop@2x.webp 2x"));
  };

  var makeSocial = function makeSocial(socialElem, socialData, socialName, sectionName, authorName) {
    var socialItem = socialElem.parent();
    var svg = socialElem.find('svg');
    var title = svg.find('title');
    socialElem.remove();
    socialElem.attr('href', socialData);
    svg.attr('aria-labelledby', "".concat(sectionName, "heading-").concat(socialElem.find('use').attr('xlink:href').split('#')[1], "-user-").concat(makeSocial.counter));
    title.text("".concat(authorName, " ").concat(socialName));
    socialElem.find('title').attr('id', "".concat(sectionName, "heading-").concat(socialElem.find('use').attr('xlink:href').split('#')[1], "-user-").concat(makeSocial.counter));
  ...
  var resetSocialCounter = function resetSocialCounter() {
    makeSocial.counter = 1;
  };

  resetSocialCounter();

  var makeTeamMember = function makeTeamMember(teamMemberData) {
  ...
  var replaceClonedSocials = function replaceClonedSocials(sectionName, slider, socialIconClass) {
    var $oldAttr = sectionName + 'heading';
    slider.find('.slick-cloned').find('.' + socialIconClass).each(function (i) {
